...
import type { SearchResult, CourseSummary } from "../types.js";

export const searchCoursesSchema = z.object({
  query: z.string().optional().describe("Search text"),
  activity_type: z.string().optional().describe("Activity type filter, e.g. 'Climbing', 'Scrambling', 'Sea Kayaking', 'Navigation'"),
  branch: z.string().optional().describe("Branch filter, e.g. 'Seattle', 'Tacoma', 'Olympia'"),
  difficulty: z.string().optional().describe("Difficulty filter: 'Casual', 'Easy', 'Moderate', 'Challenging'"),
